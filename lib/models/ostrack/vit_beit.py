...
class Attention(nn.Module):
    ...
    def __init__(
            ...
            tem_window_size: Optional[Tuple[int, int]] = None,
        ...
        self.tem_window_size = tem_window_size
        if window_size:
    ...
    def _get_rel_pos_bias_tem(self):
        tem_relative_position_bias = self.relative_position_bias_table[
            self.tem_relative_position_index.view(-1)].view(
                self.tem_window_size[0] * self.tem_window_size[1] + 1,
                self.tem_window_size[0] * self.tem_window_size[1] + 1, -1)  # Wh*Ww,Wh*Ww,nH
        tem_relative_position_bias = tem_relative_position_bias.permute(2, 0, 1).contiguous()  # nH, Wh*Ww, Wh*Ww
        return tem_relative_position_bias.unsqueeze(0)
    ...
    def forward(self, x, shared_rel_pos_bias: Optional[torch.Tensor] = None):
        ...
        if self.fused_attn:
            ...
            if self.relative_position_bias_table is not None:
                ...
                if self.tem_window_size:
                    self.tem_num_relative_distance = (2 * self.tem_window_size[0] - 1) ** 2 + 3
                    
                    self.tem_relative_position_bias_table = resize_rel_pos_bias_table_simple(
                        self.relative_position_bias_table,
                        new_window_size=self.tem_window_size,
                        new_bias_shape=(self.tem_num_relative_distance, self.num_heads)
                    ).to(self.relative_position_bias_table)
                    
                    tem_rel_pos_bias = self._get_rel_pos_bias_tem()
                    
                    all_rel_pos_bias = torch.zeros((1, self.num_heads, N, N)).to(rel_pos_bias)
                    z_len = self.tem_window_size[0] * self.tem_window_size[1]
                    all_rel_pos_bias[:, :, :z_len + 1, :z_len + 1] = tem_rel_pos_bias
                    all_rel_pos_bias[:, :, z_len + 1:, z_len + 1:] = rel_pos_bias[:, :, 1:, 1:]
                    all_rel_pos_bias[:, :, z_len + 1:, 0] = rel_pos_bias[:, :, 1:, 0]
                    all_rel_pos_bias[:, :, 0, z_len + 1:] = rel_pos_bias[:, :, 0, 1:]
                    rel_pos_bias = all_rel_pos_bias
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.attn = Attention(
            ...
            tem_window_size=tem_window_size,
...
class Beit(nn.Module):
    ...
    def __init__(
            ...
            tem_img_size: Union[int, Tuple[int, int]] = None,
    ...
    def forward_features(self, x, z, text_embed=None):
